...
class ItemAdmin @Inject() (
  ...
  def itemAddFormPage(
    form: Form[(String, String, BigDecimal, String, List[String])] = addItemForm
  )(implicit request: Request[_]): HtmlFormat.Appendable =
    indexTemplate(
      body = html.admin.newItemForm(form, categoryDAO.all().map(c => c.urlName -> c.displayName)),
